      ...
      if (prevSelected.includes(accountId)) {
        ...
        setBankAccountsData((prevData) =>
          prevData.filter(
            (item) =>
              item.id !==
              data?.data?.accounts.find((acc) => acc.id === accountId)?.number
          )
        );
  ...
  const handleSelectChange = (accountId, selectedValue) => {
    ...
    setSelectedOption((prev) => {
      ...
      setBankAccountsData((prevData) => {
        ...
        const lineOfCreditNumber = selectedLineOfCredit
          ? selectedLineOfCredit.split(" - ")[1]
          : "";

        const existingAccountIndex = prevData.findIndex(
          (item) => item.id === account?.number
        );
        const updatedAccount =
          existingAccountIndex !== -1
            ? prevData[existingAccountIndex]
            : {
                id: account?.number,
                cuenta_contable: selectedValue,
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white rounded-lg shadow-lg relative p-4 w-11/12 md:w-4/5 lg:w-2/3 xl:w-1/1">
        ...
        <div className="container overflow-x-auto overflow-y-visible">
            ...
            {currentAccounts && currentAccounts.length > 0 ? (
              currentAccounts.map((account) => {
                const options = accountingAccounts.map((cuenta) => ({
                  ...
                  disabled: Object.values(selectedOption).includes(
                    ...
                    <td className="px-6 py-4">
                      <CustomSearchSelect
                        ...
                        onChange={(selectedValue) =>
                          handleSelectChange(account.id, selectedValue)
                        }
                      />
                    </td>
                    {lineOfCredit?.length > 0 && (
                      <td className="px-6 py-4">
                        <select
                          ...
                          onChange={(event) =>
                            handleAccountSelectChange(event, account.id)
                          }
                        ...
                        >
                          ...
                          {lineOfCredit?.map((item) => (
                            <option
                              ...
                              ).includes(`${item.name} - ${item.number}`)}
                              className={
                                Object.values(selectedAccountNames).includes(
              ...
              <tr className="text-center h-32">
                <td
                  colSpan="6"
                  className="border p-2 text-center text-sm text-gray-600"
                >
