...
const CustomSelectWithoutRutMobile = ({ headlinesData, UnmatchedCount, setIsOptionSelected, handleClickDescripcion }) => {
  ...
  const handleSelectionChange = (item) => {
    ...
    setSelected(item);
    handleClickDescripcion(item.descripcion, item.date, item.monto);;
    if (item.descripcion !== '') {
              ...
              <div key={index}>
                <Listbox.Option
                  ...
                  className={({ active }) => `
                    ...
                    ${selected.descripcion === item.descripcion || active ? 'bg-customBackgroundGreen text-white' : 'text-gray-900'}
                  `}
                >
                  <div className="flex flex-col">
                    <span className="truncate text-gray-500">Nombre: {item.descripcion}</span>
                    <span className="truncate text-gray-500">Fecha: {formatDate(item.date)}</span>
